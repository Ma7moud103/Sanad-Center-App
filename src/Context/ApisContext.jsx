...
export default function ApisContextProvider(preops) {
  ...
  const getGrades = async () => {
    try {
      ...
      if (response.status === 200 || response.data.status === "success") {
        setGrades([
          {
            nameEn: t("Notifications.showAll"),
            nameAr: t("Notifications.showAll"),
            _id: "",
          },
          ...response.data.data,
        ]);
  ...
  const [handleDeleteBoockedCourse, sethandleDeleteBoockedCourse] =
    useState(false);
  ...
  const [selectedCourseINBookCourse, setselectedCourseINBookCourse] =
    useState("");
  const [selectedGroupsBookCourse, setselectedGroupsBookCourse] = useState([]);

  const cachedStudentData = JSON.parse(
    sessionStorage.getItem("studentsDetails")
  );
  const studentDetails = studentData
    ? studentData
    : cachedStudentData !== null && cachedStudentData;
  ...
  const GetAllCourses = async () => {
    if (selectedCenter) {
      try {
        const query =
          selectedGradeOfCourse.nameAr === "عرض الكل" ||
          selectedGradeOfCourse.nameAr === "Show all"
            ? ""
  ...
  const GetAllStuents = async () => {
    if (selectedCenter) {
      try {
        const query =
          studentGrade?._id !== "" ? `&grade._id=${studentGrade?._id}` : "";
        // console.log(query)
        const response = await axios.get(
  ...
  const getSessionsForGroup = async () => {
    if (selectedCenter && course && group) {
      try {
        ...
        if (res.status === 200 || res.data.status === "success") {
          setsessions([
            { name: t("Logs.selectAll"), value: "all" },
            ...res.data.message,
          ]);
  ...
  const getCenterKey = async () => {
    ...
    try {
      const res = await axios.get(`${BASE_URL}center-users/details`, {
        headers: headers,
      });
      // console.log(res)
      if (res.status === 200 || res.data.status === "success") {
  ...
  const GetAlert = async () => {
    try {
      const res = await axios.get(
        `${BASE_URL}centers/${selectedCenter?._id}/announcements?limit=6&page=${alertPage}`,
        { headers: headers }
      );
  ...
  const handleDeletBoockedCourse = async () => {
    if (
      !(
        selectedCenter &&
        boockedCourseReq.centerCourseId &&
        boockedCourseReq.cardCode &&
        boockedCourseReq.cardCourseId
      )
    )
      return null;
    ...
    if (boockedCourseReq.fire) {
      try {
        ...
        const res = await axios.delete(
          `${BASE_URL}centers/${selectedCenter?._id}/center-courses/${boockedCourseReq.centerCourseId}/cards/${boockedCourseReq.cardCode}/card-courses/${boockedCourseReq.cardCourseId}`,
          { headers: headers }
        );
  ...
  return (
    <ApisContext.Provider
      value={{
        ...
        selectedGroupsBookCourse,
        setselectedGroupsBookCourse,
        handleModifyCourse,
