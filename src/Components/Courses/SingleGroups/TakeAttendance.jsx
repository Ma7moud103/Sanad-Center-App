...
export default function TakeAttendance() {
    ...
    const { Toggler, setToggler, ErorrMessage, direction, handleUserName, cardCode, setattendHandller } = useContext(MainContext);
    ...
    const CheckCard = async () => {
        ...
        if ((groupId && selectedCenter && cardCode) && selectedSession !== null && selectedSession?.value !== "all") {

            try {
                setloading(true);
                const res = await axios.get(
                    `${BASE_URL}attendance/center/${selectedCenter?._id}/group/${groupId}?cardCode=${cardCode}&centerCourseId=${courseId}${previousSession}`,
                    { headers: headers }
                );
    ...
    const takeAttendence = async () => {
        try {
            ...
            const res = await axios.post(
                `${BASE_URL}attendance/center/${selectedCenter?._id}/group/${groupId}/`,
                attendenceDepend,
                { headers: headers }
            );
    ...
    function close() {
        ...
        setToggler({ ...Toggler, takeAttend: false });
    }
                    ...
                    <div className="flex items-center justify-center min-h-full p-4 py-6 b">
                        ...
                        >
                            <button
                                ...
                                onClick={() => setToggler({ ...Toggler, takeAttend: false })}
                            >
                                <ReactSVG src={x} />
                            </button>
                            <DialogTitle
                            ...
                            >
                                <div className="flex flex-col items-center justify-between rounded-t gap-y-1">
                                    ...
                                    <p className="text-base font-normal text-center sm:text-lg text-textColor__2">
                                        {t('homeRev.attendanceP')}
                                    </p>
                                </div>
                            </DialogTitle>
                            {/* content */}

                            <form className="relative flex flex-col items-center justify-start my-2 mt-4 gap-y-2 h-2/5">
                                {!studentDetails ? (
                                    <>


                                        <div className="flex flex-col w-full md:flex-row gap-y-3 md:items-center md:gap-x-2 ">
                                            <div className='w-full md:w-2/4'>
                                                <h5 className='text-sm font-semibold text-textColor__2'>{t("homepage.sessionSelect")}</h5>
                                                ...
                                                >
                                                    {({ open }) => (
                                                        <div className="relative w-full mt-2 ">
                                                            <Listbox.Button
                                                                className={`font-semibold bg-white px-4 py-2 text-sm 
                                                 relative w-full flex cursor-pointer rounded-lg  text-left focus:outline-none  items-center justify-between  sm:text-sm gap-x-8 `}
                                                            >
                                                                <span
                                                                    className={`  block truncate text-mainColor text-center text-xs sm:text-sm `}
                                                                >
                                                            ...
                                                            <Listbox.Options
                                                                className="absolute z-10 w-full mt-1 overflow-y-auto bg-white rounded-md shadow max-h-32 scrollbar-thin focus:outline-none"
                                                            >
                                                                {filteredSession?.map(
                                                                    (person, personIdx) => (
                                                                        <Listbox.Option
                                            ...
                                            {selectedSession !== null && selectedSession?.type && selectedSession?.type !== "" && <div className='w-full md:w-1/4'>
                                                <h5 className='text-sm font-semibold text-textColor__2'>{t('SingleCourse.sessionType')}</h5>
                                                <p className='px-3 py-2 mt-2 text-sm font-bold bg-white rounded-lg text-mainColor'>
                                            ...
                                            <div className={`w-full ${selectedSession !== null && selectedSession?.type && selectedSession?.type !== "" ? "md:w-1/4" : "md:w-2/4"}`}>
                                                <h5 className='text-sm font-semibold text-textColor__2'>{t('homepage.codeLabel')}</h5>
                                                <p className='px-3 py-2 mt-2 text-sm font-bold bg-white rounded-lg text-mainColor'>
                                        ...
                                        <div className="flex flex-col justify-between w-full gap-3 mt-4 buttons md:flex-row">
                                            <button
                                                ...
                                                disabled={!((groupId && selectedCenter && cardCode) && selectedSession !== null && selectedSession?.value !== "all")}
                                                className={`rounded-2xl ${(groupId && selectedCenter && cardCode) && selectedSession !== null && selectedSession?.value !== "all" ? "bg-mainColor" : "bg-secondMainColor"}    py-2 sm:py-3 px-12  border-none text-white text-lg w-full md:w-1/2 font-bold flex items-center justify-center`}
                                            >
                                                {!loading ? (
