...
// import leftArrow from '../../../assets/sanadSVG/singleLeftArrow.svg';
...
export default function AddGroup() {
	...
	const filteredArr = Arr.filter((item) => item.day !== selectedDay.day);
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object({
			maxStudents: Yup.number().required().integer(),
			dayOfWeek: Yup.number().required().integer(),
		...
		onSubmit: async (values, { resetForm }) => {
			...
			if (selectedCenter && courseId) {
				try {
					setMainLoading({ ...MainLoading, addGroupL: true });
					const res = await axios.post(
						`${BASE_URL}centers/${selectedCenter?._id}/center-courses/${courseId}/groups`,
						values,
						{ headers: headers }
					);
					if (res.status === 200 || res.status === 'success') {
						ErorrMessage(t('Errors.addGroup'), 'success');
						sethandleAddGroup((prev) => !prev);

						setToggler({ ...Toggler, showAddGroup: false });
						resetForm();
					}
				} catch (error) {
					console.log(error);
					...
					setMainLoading({ ...MainLoading, showAddGroup: false });
				}
			}
					...
					<div className="flex min-h-full b items-center justify-center p-4 py-8">
						...
						>
							<button
								...
								onClick={() => setToggler({ ...Toggler, showAddGroup: false })}
							>
								<ReactSVG src={x} />
							</button>
							<DialogTitle
							...
							>
								<div className="flex flex-col gap-2 w-full">
									...
									<div className="flex bg-white justify-between items-center rounded-2xl py-2 sm:py-3 px-5 shadow">
										<div className="flex gap-2 items-center justify-center">
											<span className="cursor-pointer">
												{/* <img
													className="w-8 h-8"
													src={
														courseDetails?.tutorCourse?.courseData?.image
															? courseDetails?.tutorCourse?.courseData?.image
															: avatar
													}
													alt=""
											...
											<div className="flex flex-col">
												<p className="font-bold text-mainColor text-size_12 sm:text-sm  flex justify-start items-center gap-2">
													{courseDetails?.tutorCourse?.courseData?.name}
												</p>
										...
										<div className="flex flex-col sm:flex-row sm:gap-x-3">
											...
											<p className="text-xs sm:text-sm text-[#9CA3AF]">
												{courseDetails?.tutorCourse?.tutor?.fullname}
											</p>
										</div>
									</div>
								</div>
								...
								<div className="flex flex-col sm:flex-row gap-2 items-center">
									<div className="md:w-2/3 w-full sm:w-1/2 flex flex-col gap-y-1  relative z-[59]">
										<label
											htmlFor="day"
										...
										<Listbox
											...
											onChange={(value) => {
												setselectedDay({ ...selectedDay, day: value.day });
                      ...
                      relative w-full flex cursor-pointer rounded-lg bg-white text-left focus:outline-none  sm:text-sm`}
                        ...
                        scrollbar-thin  w-full overflow-y-scroll rounded-md bg-white py-1 text-base  ring-1 ring-black/5 focus:outline-none sm:text-sm shadow"
									...
									<div className="flex flex-col md:w-2/3 w-full sm:w-1/2  gap-y-1">
										<label
											htmlFor="maxStudents"
											className="text-mainColor w-full text-start font-semibold text-xs relative flex justify-start items-baseline"
										>
                        ...
                        border-[#E6E9EA]
                           outline-none  text-start border-[1px] rounded-xl focus:border-mainColor border-solid focus:border-[1px] placeholder:text-start focus:shadow-none placeholder:text-size_12 placeholder:sm:text-smn text-size_12 sm:text-sm  ${formik.errors.maxStudents ? 'text-err border border-err outline-none focus:outline-none placeholder:text-err' : 'text-mainColor'}`}
								...
								<div className="formBtns  mt-2 flex  gap-x-3 justify-center items-center">
									<button
										type="submit"
										disabled={isButtonDisabled}
										className={`${isButtonDisabled ? 'bg-secondMainColor' : 'bg-mainColor'} text-white rounded-2xl px-10 flex items-center justify-center text-base py-2 sm:py-3  w-full md:w-1/2 sm:text-xl`}
									>
									...
									<button
										...
										onClick={() => {
											setToggler((prev) => {
												return { ...prev, showAddGroup: false };
											});
										}}
										className="bg-transparent text-mainColor rounded-2xl px-10 text-base py-2 sm:py-3 w-full md:w-1/2 sm:text-xl"
									>
										{t('PopupWindows.back')}
									</button>
								</div>
